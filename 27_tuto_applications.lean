...
-- Montrez que l'identité est surjective
  theorem identite_surjective  : sorry  := sorry
    sorry

-- Exercice 2.3
-- Montrez que l'identité est bijective
  theorem identite_bijective  : sorry  := sorry
    sorry
...
namespace solution
  ...
  λ  (u: E → F) (v: F → G) ↦
    λ (h_v_o_u_injective: injective (v ∘  u)) ↦
      λ (x x':E) ↦
        λ  (h_ux_eq_ux':u x = u  x') ↦
          have h_vux_eq_vux' : v ( u x) = v (u x') :=  congr_arg v h_ux_eq_ux'
          h_v_o_u_injective x x' h_vux_eq_vux'

  theorem ex_6_4_21_2 : ∀  (u: E → F) (v: F → G), (injective (v ∘  u) )→ (surjective u)→ (injective v)  :=
  λ  (u: E → F) (v: F → G) ↦
    λ (h_v_o_u_injective: injective (v ∘  u) )↦
      λ (h_u_surjective : surjective u)↦
        λ (y y':F)↦
          λ  (h_vy_eq_vy':v y = v y')↦
            Exists.elim (h_u_surjective y) (
              λ (x:E) (h_y_eq_ux : y = u x)↦
                Exists.elim (h_u_surjective y') (
                  λ (x':E) (h_y'_eq_ux' : y' = u x') ↦
                    have h_vux_eq_vux' : v (u x)  = v (u x') :=   h_y'_eq_ux' ▸ h_y_eq_ux ▸ h_vy_eq_vy'
                    have h_x_eq_x'     : x=x'                :=   h_v_o_u_injective x x' h_vux_eq_vux'
  ...
  λ  (u: E → F) (v: F → G) ↦
    λ h_v_o_u_surjective : surjective (v ∘  u)   ↦
      λ (z:G) ↦
        Exists.elim (h_v_o_u_surjective z) (
          λ (x:E) (h_z_eq_v_o_u_x : z = (v ∘  u) x) ↦
            Exists.intro (u x) (
  ...
  λ  (u: E → F) (v: F → G) ↦
    λ h_v_o_u_surjective : surjective (v ∘  u) ↦
      λ h_v_injective: injective v ↦
        λ (y:F) ↦
          let z:= v y
          Exists.elim (h_v_o_u_surjective z)
          (
            λ (x:E) (h_z_eq_v_u_x: z = v (u x))  ↦
              Exists.intro x (
...
theorem eee : ∀ (f: E → F), ∀ (B:Set F), ∀x:E, x∈(image_reciproque f B) ↔ f x ∈ B := by aesop
#print eee

--#print image_reciproque.equations._eqn_1
--#print image_directe.equations._eqn_1
#print congr_arg
#print congr
#print Eq.subst
#print Eq.rec

  lemma set_comp (A:Set E) (x:E) : A x → x ∈ {t:E | A t} := λ h ↦  h
  #print set_comp
  ...
  theorem ex_6_3_19_3' : ∀ (f: E → F),  ∀ (A:Set E), (A ⊆  (image_reciproque f (image_directe f A))) :=
    λ (f: E → F) ↦
        λ (A:Set E) ↦
          ...
          have h1: f x ∈ (image_directe f A)                     := h0 --(image_directe.equations._eqn_1 f A).symm ▸ h0
          have h2: x   ∈ {t : E | f t ∈ (image_directe f A) }    := h1
          show x ∈(image_reciproque f (image_directe f A))       from
            --(image_reciproque.equations._eqn_1 f (image_directe f A)).symm ▸ h2
            h2
/-
  theorem ex_6_3_19_3'' : ∀ (f: E → F),  ∀ (A:Set E), (A ⊆  (image_reciproque f (image_directe f A))) :=
    λ (f: E → F) ↦
        λ (A:Set E) ↦
          ...
          show x ∈(image_reciproque f (image_directe f A)) from
--             calc
--                x    ∈  {t : E | f t ∈ (image_directe f A) }     :  h1
--                ...  =  (image_reciproque f (image_directe f A)) :  (image_reciproque.equations._eqn_1 f (image_directe f A)).symm
          @Eq.rec
             (Set E)
             {t : E | f t ∈ (image_directe f A) }
             (λ B ↦ x ∈ B )
             h2
             (image_reciproque f (image_directe f A))
             rfl --(image_reciproque.equations._eqn_1 f (image_directe f A)).symm

  theorem ex_6_3_19_3''' : ∀ (f: E → F),  ∀ (A:Set E), (A ⊆  (image_reciproque f (image_directe f A))) :=
    λ (f: E → F) ↦
        λ (A:Set E) ↦
          λ x:E ↦ λ (h_xA: x ∈ A) ↦
          have h1: f x ∈ (image_directe f A)                   := Exists.intro x ⟨ h_xA,  (Eq.refl (f x)) ⟩
          have h2:  x    ∈  {t : E | f t ∈ (image_directe f A) } :=  h1
          show x ∈(image_reciproque f (image_directe f A)) from
          @Eq.rec
             _
  ...
  theorem ex_6_3_19_3'''' : ∀ (f: E → F),  ∀ (A:Set E), (A ⊆  (image_reciproque f (image_directe f A))) :=
    λ (f: E → F),
        λ (A:Set E),
          λ x:E ↦ λ (h_xA: x ∈ A),
          have h1: f x ∈ (image_directe f A),                     from Exists.intro x ⟨ h_xA,  (Eq.refl (f x)) ⟩,
          have h2:  x    ∈  {t : E | f t ∈ (image_directe f A) } :=  h1
          show x ∈(image_reciproque f (image_directe f A)) from
          @eq.subst
             (Set E)
             ...
             {t : E | f t ∈ (image_directe f A) }
             (image_reciproque f (image_directe f A))
             (image_reciproque.equations._eqn_1 f (image_directe f A)).symm
             h2
    ...
    λ (f: E → F) ↦
      λ (h_forall: ∀ (H:Set F), H ⊆  (image_directe f (image_reciproque f H))) ↦
        λ (y:F) ↦
          ...
          have h_imrec_nonempty : Set.Nonempty (image_reciproque f H) :=  ex_6_3_20_1_3 f  (image_reciproque f H) h_im_nonempty
  ...
  lemma ex_6_3_20_2_1' : ∀  (f: E → F), (injective f) → (∀ (A:Set E), ((image_reciproque f (image_directe f A)) ⊆ A)) :=
    λ (f: E → F) ↦
      λ (h_inj: injective f) ↦
        λ (A:Set E) ↦
          λ x:E ↦ λ (h_xfm1fA: x ∈ (image_reciproque f (image_directe f A))) ↦
          Exists.elim h_xfm1fA (
            λ(x':E) ↦
              λ ( h_x'A_and_fxfx' : (x'∈ A) ∧( f x = f x')) ↦
                @Eq.subst _ (·∈ A ) _ _
                    (Eq.symm (h_inj x x' h_x'A_and_fxfx'.right))
                    h_x'A_and_fxfx'.left
          )
  ...
  lemma ex_6_3_20_2_1'' : ∀  (f: E → F), (injective f) → (∀ (A:Set E), ((image_reciproque f (image_directe f A)) ⊆ A)) :=
    λ _ h1 A x h2 ↦
          Exists.elim h2 (
            λ x' h3 ↦
                @Eq.subst _ (· ∈ A) _ _
                    (Eq.symm (h1 x x' h3.right))
                    h3.left
          )

  lemma ex_6_3_20_2_1''' (f: E → F) :  (injective f) → (∀ (A:Set E), ((image_reciproque f (image_directe f A)) ⊆ A)) :=
    λ  h1 _ x ⟨ x',⟨ h4,h5⟩ ⟩ ↦  (h1 x x' h5).symm  ▸ h4
              ...
              (λ (h:y ∈ image_directe  f {x}) ↦
                  Exists.elim h (
                    λ (t:E) ↦ λ (h_t:t ∈ {x} ∧ y= f t) ↦
                      have ht3 : _ := Set.eq_of_mem_singleton h_t.left
                      (((Set.eq_of_mem_singleton h_t.left) ▸ h_t.right) : y = f x)
--                      have ht1 : _ := (Set.eq_of_mem_singleton (h_t.left: t ∈ {x} ) : t=x) ▸ h_t.right
--                      have ht2 : t ∈ {x} := h_t.left
--                      have ht3 : _ := Set.eq_of_mem_singleton ht2
--                      (((Set.eq_of_mem_singleton h_t.left) ▸ h_t.right) : y = f x)
--                        (Set.eq_of_mem_singleton (h_t.left: t ∈ {x} ) : t=x) ▸ h_t.right
--                      ((ht3 ▸ h_t.right) : y = f x)
                  )
              )
--              (λ (h:y ∈ image_directe  f {x}), Exists.elim h (λ (t:E), λ (h_t:t∈ {x} ∧ y= f t),  h_t.left ▸ h_t.right))
--              (λ (h:y ∈ {f x})  ↦Exists.intro x (And.intro ((Eq.refl x) : (x∈ ({x}:(Set E)))) (h: y = f x)  ))
              (λ (h:y ∈ {f x})  ↦Exists.intro x (And.intro ((Set.mem_singleton x) : (x∈ {x})) (h: y = f x)  ))
--              (λ (h:y ∈ {f x})  ↦Exists.intro x (And.intro (set.mem_singleton x)  (h: y = f x)  ))
--              (λ (h:y ∈ {f x})  ↦Exists.intro x (And.intro (Eq.refl x)  (h: y = f x)  ))
        )

  lemma ex_6_3_20_2_2' : ∀  (f: E → F), ∀ (x:E), (image_directe  f {x}) = {f x} := by
     intros;   ext1; fconstructor ;   intro h ; cases h ; aesop ;  intro h ;  fconstructor ;  tauto ; aesop
    ...
    λ (f: E → F) ↦
      λ (h_forallset: ∀ (A:Set E), (image_reciproque f (image_directe f A)) ⊆ A)  ↦
        λ (x:E) (x':E) (h_fx1x' : f x = f x') ↦
          let A := ({x}:Set E)
          have h_fm1fA : image_reciproque f {f x} ⊆ {x} :=   (ex_6_3_20_2_2 f x) ▸  (h_forallset A)
  ...
  theorem ex_6_3_20_2 :  ∀  (f: E → F), (injective f) ↔  (∀ (A:Set E), ((image_reciproque f (image_directe f A)) = A)) :=
     λ (f: E → F) ↦
       Iff.intro
       (λ (h_inj: injective f) ↦ λ (A:Set E) ↦ subset_antisymm (ex_6_3_20_2_1 f h_inj A) (ex_6_3_19_3 f A))
       (λ (h_forallset: ∀ (A:Set E), (image_reciproque f (image_directe f A)) = A)  ↦ex_6_3_20_2_3 f (
...
--                     Exists.elim (h_existsH) (
  ...
  theorem ex_6_3_20_4 :
    ...
    λ (f:E → F) ↦
      Iff.intro
      (λ (h_f_injective : injective f) ↦
        λ (A:Set E) ↦
          λ (B:Set E) ↦
            subset_antisymm
              (ex_6_2_18_3 f A B)
              (
                λ (y:F) (h_y_in_inter : y ∈ (image_directe f A)∩ (image_directe f B) ) ↦
                  Exists.elim h_y_in_inter.left  (λ (xA:E) (h_xA_and: xA ∈ A ∧ y = f xA )  ↦
                  Exists.elim h_y_in_inter.right (λ (xB:E) (h_xB_and: xB ∈ B ∧ y = f xB )  ↦
                  have h_fxA_eq_fxB  : f xA = f xB :=  h_xA_and.right ▸ h_xB_and.right
                  have h_xA_eq_xB    :   xA = xB := h_f_injective xA xB h_fxA_eq_fxB
                  Exists.intro xA (
                    And.intro
      ...
      (λ (h_forallsets : ∀ A: Set E, ∀ B: Set E, image_directe f (A∩B) = (image_directe f A)∩ (image_directe f B)) ↦
        λ (x x':E) (h_fx_eq_fx' : f x = f x') ↦
         let A:Set E := {x} ; let B:Set E:={x'} ;
         have h_fx_in_inter : f x ∈ (image_directe f A)∩ (image_directe f B) :=
             And.intro
         ...
         have h_fAB_nonempty: Set.Nonempty (image_directe f (A∩B)) :=
           Exists.intro (f x) (
              (h_forallsets A B).symm ▸ h_fx_in_inter
           )
         have h_A_inter_B_nonempty : Set.Nonempty (A∩B) :=  ex_6_3_20_1_3 f (A∩B) h_fAB_nonempty
...
lemma ex_6_3_20_5_1 : ∀ f:E→ F, image_directe f ∅ = ∅ := λ (f:E→ F) ↦  (Set.subset_empty_iff).mp ( -- (@set.subset_empty_iff F (image_directe f ∅)).mp (
            ...
            (
               (reformulation_surjectivite f).mpr (
                  calc
                    ...
                    _                = (∅: Set F)ᶜ             := congr_arg HasCompl.compl (ex_6_3_20_5_1 f)
