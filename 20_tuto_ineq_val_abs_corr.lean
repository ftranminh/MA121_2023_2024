...
example  : ∀ (x :ℝ), x-(x-1) ≤ 2  ↔ True :=
  λ (x :ℝ) ↦
    calc
      x-(x-1) ≤ 2  ↔ 1 ≤ 2 :=  iff_arg  (· ≤ 2 ) (by ring_nf : x - (x - 1) = 1 )
      _            ↔ True  := by norm_num

-- Exercice 0.2

example  : ∀ (x :ℝ), -x-(-(x-1)) > 0   ↔ False :=
  λ (x :ℝ) ↦
    calc
      -x-(-(x-1)) > 0  ↔ -1 > 0  := iff_arg  (·  > 0 ) (by ring_nf : -x-(-(x-1)) = -1)
      _                 ↔ False  := by norm_num
...
-- pour prouver que   a ≤ b ↔ a-c ≤ b-c   on peut utiliser  (add_le_add_iff_right (-c)).symm
-- mais il existe aussi des lemmes spécifiques pour les soustractions
  example (a b c :ℝ ) : a ≤ b ↔ a-c ≤ b-c := (add_le_add_iff_right (-c)).symm
  example (a b c :ℝ ) : a ≤ b ↔ a-c ≤ b-c := (sub_le_sub_iff_right c).symm
  example (a b c :ℝ ) : a < b ↔ a-c < b-c := (sub_lt_sub_iff_right c).symm
...
-- Exercice 1.1
  ...
  example : ∀ x:ℝ , x+2 ≥ -3  ↔  x ≥ -5 :=
    λ x:ℝ ↦
      calc
        x+2 ≥ -3 ↔ x+2-2 ≥ -3-2  := (add_le_add_iff_right (-2)).symm
        _        ↔ x ≥ -5        := by ring_nf


-- Exercice 1.2

  example : ∀ x:ℝ , x-3 < -1   ↔  x < 2 :=
    λ x:ℝ ↦
      calc
        x-3 < -1  ↔  x-3+3 < -1+3  := (add_lt_add_iff_right 3).symm
        _         ↔  x < 2         := by ring_nf

-- Exercice 1.3

  example : ∀ x:ℝ , -(x-1) ≥ 12  ↔  x ≤ -11 :=
    λ x:ℝ ↦
      calc
        -(x-1) ≥ 12  ↔  (x-12)-(x-1) ≥ (x-12)+12  := (add_le_add_iff_left (x-12)).symm
        _            ↔ -11 ≥ x                    := by ring_nf


-- Exercice 1.4

  example : ∀ x:ℝ , 1+x/3+1+2*x/3+1 < -1/3  ↔  x < -10/3 :=
    λ x:ℝ ↦
      calc
        1+x/3+1+2*x/3+1 < -1/3  ↔  -3+(1+x/3+1+2*x/3+1) < -3+(-1/3)  := (add_lt_add_iff_left (-3)).symm
        _                       ↔  x < -10/3                         := by ring_nf
...
-- Exercice 2.1
  example : ∀ x:ℝ , x - (x/2-1) > 0  ↔  x > -2 :=
    λ x:ℝ ↦
      calc
        x - (x/2-1)  > 0  ↔  x/2+1   > 0       := by ring_nf
        _                 ↔  x/2+1-1 > 0-1     := (add_lt_add_iff_right (-1)).symm
        _                 ↔  x/2     > -1      := by ring_nf
        _                 ↔  x/2*2   > -1*2    := (mul_lt_mul_right (by norm_num : 2>(0:ℝ))).symm
        _                 ↔  x > -2            := by ring_nf

-- Exercice 2.2
  example : ∀ x:ℝ , -3*x+4 ≤ 1  ↔  x ≥ 1  :=
    λ x:ℝ ↦
      calc
       -3*x+4 ≤ 1  ↔ -3*x+4-4 ≤ 1-4  := (sub_le_sub_iff_right 4).symm
       _          ↔  (-3)*x ≤ (-3)*1 := by ring_nf
       _          ↔  x    ≥  1       := (mul_le_mul_left_of_neg (by norm_num : -3 < (0:ℝ)))

  example : ∀ x:ℝ , -3*x+4 ≤ 1  ↔  x ≥ 1  :=
    λ x:ℝ ↦
      calc
       -3*x+4 ≤ 1  ↔ -3*x+4+(3*x-1) ≤ 1+(3*x-1) := (add_le_add_iff_right (3*x-1)).symm
       _          ↔  3*1 ≤ 3*x                  := by ring_nf
       _          ↔  1  ≤   x                   := (mul_le_mul_left (by norm_num : 3 > (0:ℝ)))
...
namespace ineq_val_abs  -- création d'un espace de noms pour ne pas entrer en conflit avec  le   abs   de la Mathib
  ...
  example : abs 0 = 0        := abs_zero
  example : abs (3-1) = 2    :=
    calc
      abs (3-1) = abs 2  := by norm_num
      _         = 2      := abs_of_nonneg (by norm_num : 2 ≥ (0:ℝ))


  example : abs (1-3) = 2    :=
    calc
      abs (1-3) = abs (-2) := by norm_num
      _         = -(-2)    := abs_of_nonpos (by norm_num : (-2) ≤ (0:ℝ))
      _         = 2        := by norm_num
  ...
  example : ∀ x:ℝ , abs (3-x/2) = abs (x/2-3) :=
    λ x:ℝ ↦
      calc
        abs (3-x/2) = abs (-(3-x/2)) := (abs_neg_eq_abs (3-x/2)).symm
        _         = abs (x/2-3)      := by ring_nf
...
namespace ineq_val_abs
  ...
  -- Le petit lemme ci-dessous est utile  pour  mettre en place de disjonctions de 3 cas
    ...
    -- Exercice 4.1
    example  : ∀ x:ℝ , abs (x+1) + abs (2-x) ≥ 3 :=
      λx:ℝ ↦

        have he : abs (x+1) + abs (2-x)  =  abs (x+1) + abs (x-2) :=
          calc
            abs (x+1) + abs (2-x) = abs (x+1) + abs (-(2-x))  := by rw[abs_neg_eq_abs]
            _                     = abs (x+1) + abs (x-2)     := by ring_nf

        Or.elim3 (left_or_center_or_right x (-1) 2)
        (
           λ h : x < -1 ↦
             have h0 : x+1 ≤ 0 := le_trans ( (add_le_add_right (le_of_lt h) _) ) (by norm_num)   -- by linarith
             have h1 : x-2 ≤ 0 := le_trans ( (add_le_add_right (le_of_lt h) _) ) (by norm_num)   -- by linarith

             calc
               abs (x+1) + abs (2-x)  =  abs (x+1) + abs (x-2)  := he
               _                      = -(x+1) + (-(x-2))       := by rw[abs_of_nonpos h0,  abs_of_nonpos h1]
               _                      = (-2)*x+1                := by ring_nf
               _                      ≥ (-2)*(-1)+1             := add_le_add_right (mul_le_mul_of_nonpos_left (le_of_lt h) (by norm_num) )  (1:ℝ)
               _                      = 3                       := by norm_num
        )
        (
           λ h : x ≥ -1  ∧ x < 2  ↦
             have h0 : x+1 ≥ 0 := le_trans (by norm_num) ( (add_le_add_right h.left _) )               -- by linarith
             have h1 : x-2 ≤ 0 := le_trans ( (add_le_add_right (le_of_lt h.right) _) ) (by norm_num)   -- by linarith

             calc
               abs (x+1) + abs (2-x)  =  abs (x+1) + abs (x-2)  := he
               _                      = (x+1) + (-(x-2))        := by rw[abs_of_nonneg h0,  abs_of_nonpos h1]
               _                      = 3                       := by ring_nf
               _                      ≥ 3                       := by norm_num
)
        (
           λ h : x ≥ 2 ↦
             have h0 : x+1 ≥ 0 := le_trans (by norm_num) ( (add_le_add_right h _) )    -- by linarith
             have h1 : x-2 ≥ 0 := le_trans (by norm_num) ( (add_le_add_right h _) )    -- by linarith

             calc
               abs (x+1) + abs (2-x)  =  abs (x+1) + abs (x-2)  := he
               _                      =   (x+1)  + (x-2)        := by rw[abs_of_nonneg h0,  abs_of_nonneg h1]
               _                      = 2*x-1      := by ring_nf
               _                      ≥ 2*2-1      := add_le_add_right (mul_le_mul_of_nonneg_left h (by norm_num) )  ((-1):ℝ) --by rel[h]
               _                      = 3          := by norm_num
        )
    ...
    -- Exercice 4.2
    example (x:ℝ ) : abs (3-2*x) - abs (2*x) < 4 ↔ True  :=
      Or.elim3 (left_or_center_or_right x  0 (3/2) )
      (
        λ h : x < 0 ↦
          have h1 : 3-2*x ≥ 0 := by linarith
          have h2 : 2*x   ≤ 0 := by linarith
          calc
             abs (3-2*x) - abs (2*x) < 4 ↔ (3-2*x) -  (-(2*x)) < 4 := by rw[abs_of_nonneg h1, abs_of_nonpos h2 ]
             _                           ↔ 3 < 4                := by norm_num
             _                           ↔ True                 := Iff.intro (λ _ ↦ trivial) (λ_ ↦ (by norm_num) )
      )
      (
        λ h : x ≥ 0 ∧ x < 3/2 ↦
          have h1 : 3-2*x ≥ 0 := by linarith
          have h2 : 2*x   ≥ 0 := by linarith
          calc
             abs (3-2*x) - abs (2*x) < 4 ↔ (3-2*x) -  (2*x) < 4           := by rw[abs_of_nonneg h1, abs_of_nonneg h2 ]
             _                           ↔ 3-4*x < 4                      := by ring_nf
             _                           ↔ 3-4*x + (4*x-4) < 4 + (4*x-4)  := (add_lt_add_iff_right _).symm
             _                           ↔ -1 < 4*x                       := by ring_nf
             _                           ↔ -1/4 < 4*x/4                   := (div_lt_div_right (by norm_num)).symm
             _                           ↔ -1/4 < x                       := by ring_nf
             _                           ↔ True                           := Iff.intro (λ _ ↦ trivial) (λ_ ↦ lt_of_lt_of_le (by norm_num) h.left )
      )
      (
        λ h : x ≥ 3/2  ↦
          have h1 : 3-2*x ≤ 0 := by linarith
          have h2 : 2*x   ≥ 0 := by linarith
          calc
             abs (3-2*x) - abs (2*x) < 4 ↔ -(3-2*x) - ( 2*x)  < 4 := by rw[abs_of_nonpos h1, abs_of_nonneg h2 ]
             _                           ↔ -3 < 4                 := by norm_num
             _                           ↔ True                   := Iff.intro (λ _ ↦ trivial) (λ_ ↦ (by norm_num) )
      )

    -- Exercice 4.3
    example (x:ℝ ) : abs (x) + abs (x+1) ≤ 2 ↔ x ≥ -3/2 ∧  x ≤  1/2  :=
      Or.elim3 (left_or_center_or_right x (-1) 0)
      (
        λ h : x < -1 ↦
          have h1 : x    ≤  0  := le_trans (le_of_lt h) (by norm_num )                      -- by linarith
          have h2 : x+1  ≤  0  := le_trans (add_le_add_right (le_of_lt h) _) (by norm_num ) -- by linarith
          calc
             abs (x) + abs (x+1) ≤ 2   ↔  -x + (-(x+1))     ≤ 2           := by rw[abs_of_nonpos h1, abs_of_nonpos h2 ]
             _                         ↔ -2*x-1             ≤ 2           := by ring_nf
             _                         ↔ -2*x-1 + (2*x-2)   ≤ 2 + (2*x-2) := (add_le_add_iff_right _).symm
             _                         ↔ -3                 ≤ 2*x         := by ring_nf
             _                         ↔ -3/2               ≤ 2*x/2       := (div_le_div_right (by norm_num)).symm
             _                         ↔ -3/2               ≤ x           := by ring_nf
             _                         ↔  x ≥ -3/2 ∧  x ≤  1/2            := Iff.intro (And.intro · (le_trans h1 (by norm_num) ) ) (And.left)
      )
      (
        λ h : x ≥ -1 ∧ x < 0 ↦
          have h1 : x    ≤  0  := le_of_lt h.right                                     -- by linarith
          have h2 : x+1  ≥  0  := le_trans (by norm_num )  (add_le_add_right h.left _) -- by linarith
          calc
              abs (x) + abs (x+1) ≤ 2  ↔ -x + (x+1)     ≤ 2     := by rw[abs_of_nonpos h1, abs_of_nonneg h2 ]
             _                         ↔ 1 ≤ 2                  := by norm_num
             _                         ↔ True                   := Iff.intro (λ _ ↦ trivial) (λ_ ↦ (by norm_num) )
             _                         ↔  x ≥ -3/2 ∧  x ≤  1/2  := Iff.intro (λ _ ↦ And.intro (le_trans (by norm_num) h.left) (le_trans h1 (by norm_num)) )  (λ _ ↦ trivial)
      )
      (
        λ h : x ≥ 0  ↦
          have h1 : x    ≥   0  := h                                              -- by linarith
          have h2 : x+1  ≥   0  := le_trans (by norm_num) (add_le_add_right h _)  -- by linarith
          calc
             abs (x) + abs (x+1) ≤ 2 ↔  x + (x+1)       ≤ 2     := by rw[abs_of_nonneg h1, abs_of_nonneg h2 ]
             _                         ↔ 2*x+1          ≤ 2     := by ring_nf
             _                         ↔ 2*x+1-1        ≤ 2-1   := (add_le_add_iff_right _).symm
             _                         ↔ 2*x            ≤ 1     := by ring_nf
             _                         ↔ 2*x/2          ≤ 1/2   := (div_le_div_right (by norm_num)).symm
             _                         ↔ x              ≤ 1/2   := by ring_nf
             _                         ↔  x ≥ -3/2 ∧  x ≤  1/2  := Iff.intro (And.intro (le_trans (by norm_num) h) · ) (And.right)
      )

    -- Exercice 4.4
    example (x:ℝ ) : abs (2*x)-abs (1-x/2) ≥ 2 ↔ x ≤  -2 ∨   x ≥  6/5  :=
      have h0 : abs (2*x)-abs (1-x/2) ≥ 2  ↔  abs (2*x)-abs (x/2-1) ≥ 2 :=
        calc
          abs (2*x)-abs (1-x/2) ≥ 2   ↔ abs (2*x) - (abs (-(1-x/2)))  ≥ 2  := by rw[abs_neg_eq_abs]
          _                           ↔ abs (2*x) - abs (x/2- 1)      ≥ 2  := by ring_nf

      Or.elim3 (left_or_center_or_right x 0 2)
      (
        λ h : x < 0 ↦
          have h1 : 2*x      ≤  0  := le_trans (mul_le_mul_of_nonneg_left (le_of_lt h) (by norm_num))  (by norm_num)                       -- by linarith
          have h2 : x/2 - 1  ≤  0  := le_trans (add_le_add_right (div_le_div_of_le_of_nonneg (le_of_lt h) (by norm_num)) _) (by norm_num ) -- by linarith
          calc
             abs (2*x)-abs (1-x/2) ≥ 2 ↔  abs (2*x)-abs (x/2-1) ≥ 2                := h0
             _                         ↔  -(2*x) - (-(x/2-1))   ≥ 2                := by rw[abs_of_nonpos h1, abs_of_nonpos h2 ]
             _                         ↔ -3*x/2-1               ≥ 2                := by ring_nf
             _                         ↔ -3*x/2-1 + (3*x/2 - 2) ≥ 2 + (3*x/2 - 2)  := (add_le_add_iff_right _).symm
             _                         ↔ -3                     ≥ 3*x/2            := by ring_nf
             _                         ↔ -3*(2/3)               ≥  3*x/2*(2/3)     := (mul_le_mul_right (by norm_num)).symm
             _                         ↔ -2                     ≥  x               := by ring_nf
             _                         ↔  x ≤  -2 ∨   x ≥  6/5                     := Iff.intro Or.inl (λ s ↦ Or.elim s id (λ s2 ↦ absurd (lt_of_le_of_lt s2 h) (by norm_num) ))
      )
      (
        λ h : x ≥ 0 ∧ x < 2 ↦
          have h1 : 2*x      ≥  0  := le_trans  (by norm_num)  (mul_le_mul_of_nonneg_left h.left (by norm_num))                                      -- by linarith
          have h2 : x/2 - 1  ≤  0  := le_trans (add_le_add_right (div_le_div_of_le_of_nonneg (le_of_lt h.right) (by norm_num)) _) (by norm_num ) -- by linarith
          calc
             abs (2*x)-abs (1-x/2) ≥ 2 ↔  abs (2*x)-abs (x/2-1)    ≥ 2                := h0
             _                         ↔ (2*x) - (-(x/2-1))        ≥ 2                := by rw[abs_of_nonneg h1, abs_of_nonpos h2 ]
             _                         ↔ 5*x/2-1                   ≥ 2                := by ring_nf
             _                         ↔ 5*x/2-1+1                 ≥ 2+1              := (add_le_add_iff_right _).symm
             _                         ↔ 5*x/2                     ≥ 3                := by ring_nf
             _                         ↔ 5*x/2*(2/5)               ≥ 3*(2/5)          := (mul_le_mul_right (by norm_num)).symm
             _                         ↔ x                         ≥ 6/5              := by ring_nf
             _                         ↔  x ≤  -2 ∨   x ≥  6/5                        := Iff.intro Or.inr (λ s ↦ Or.elim s  (λ s2 ↦ absurd (le_trans h.left s2) (by norm_num) ) id)
      )
      (
        λ h : x ≥ 2  ↦
          have h1 : 2*x      ≥   0  := le_trans (by norm_num) (mul_le_mul_of_nonneg_left h (by norm_num) )                                              -- by linarith
          have h2 : x/2 - 1  ≥   0  := le_trans (by norm_num) (add_le_add_right (div_le_div_of_le_of_nonneg  h (by norm_num)) _)  -- by linarith
          calc
             abs (2*x)-abs (1-x/2) ≥ 2 ↔  abs (2*x)-abs (x/2-1)    ≥ 2         := h0
             _                         ↔   2*x - (x/2-1)           ≥ 2         := by rw[abs_of_nonneg h1, abs_of_nonneg h2 ]
             _                         ↔ 3*x/2+1                   ≥ 2         := by ring_nf
             _                         ↔ 3*x/2+1-1                 ≥ 2-1       := (add_le_add_iff_right _).symm
             _                         ↔ 3*x/2                     ≥ 1         := by ring_nf
             _                         ↔ 3*x/2*(2/3)               ≥ 1*(2/3)   := (mul_le_mul_right (by norm_num)).symm
             _                         ↔ x                         ≥ 2/3       := by ring_nf
             _                         ↔ True                                  := Iff.intro (λ _ ↦ trivial) (λ _ ↦ le_trans (by norm_num) h )
             _                         ↔  x ≤  -2 ∨   x ≥  6/5                 := Iff.intro (λ _ ↦ Or.inr (le_trans (by norm_num) h)) (λ _ ↦ trivial)
      )

    -- Exercice 4.5
    example (x:ℝ ) : abs (2*x)-abs (1-x/2) > 6 ↔ x <  -14/3 ∨   x >  10/3  :=
      have h0 : abs (2*x)-abs (1-x/2) > 6  ↔  abs (2*x)-abs (x/2-1) > 6 :=
        calc
          abs (2*x)-abs (1-x/2) > 6   ↔ abs (2*x) - (abs (-(1-x/2)))  > 6  := by rw[abs_neg_eq_abs]
          _                           ↔ abs (2*x) - abs (x/2- 1)      > 6  := by ring_nf
      ...
      (
        λ h : x < 0 ↦
          ...
          have h2 : x/2 - 1  ≤  0  := le_trans (add_le_add_right (div_le_div_of_le_of_nonneg (le_of_lt h) (by norm_num)) _) (by norm_num ) -- by linarith
          calc
             abs (2*x)-abs (1-x/2) > 6 ↔  abs (2*x)-abs (x/2-1) > 6                := h0
             _                         ↔  -(2*x) - (-(x/2-1))   > 6                := by rw[abs_of_nonpos h1, abs_of_nonpos h2 ]
             _                         ↔ -3*x/2-1               > 6                := by ring_nf
             _                         ↔ -3*x/2-1 + (3*x/2 - 6) > 6 + (3*x/2 - 6)  := (add_lt_add_iff_right _).symm
             _                         ↔ -7                     > 3*x/2            := by ring_nf
             _                         ↔ -7*(2/3)               >  3*x/2*(2/3)     := (mul_lt_mul_right (by norm_num)).symm
             _                         ↔ -14/3                  >  x               := by ring_nf
             _                         ↔  x <  -14/3 ∨   x >  10/3                 := Iff.intro Or.inl (λ s ↦ Or.elim s id (λ s2 ↦ absurd (lt_trans s2 h) (by norm_num) ))
      )
      (
        λ h : x ≥ 0 ∧ x < 2 ↦
          have h1 : 2*x      ≥  0  := le_trans  (by norm_num)  (mul_le_mul_of_nonneg_left h.left (by norm_num))                                      -- by linarith
          ...
          calc
             abs (2*x)-abs (1-x/2) > 6 ↔  abs (2*x)-abs (x/2-1)    > 6                := h0
             _                         ↔ (2*x) - (-(x/2-1))        > 6                := by rw[abs_of_nonneg h1, abs_of_nonpos h2 ]
             _                         ↔ 5*x/2-1                   > 6                := by ring_nf
             _                         ↔ 5*x/2-1+1                 > 6+1              := (add_lt_add_iff_right _).symm
             _                         ↔ 5*x/2                     > 7                := by ring_nf
             _                         ↔ 5*x/2*(2/5)               > 7*(2/5)          := (mul_lt_mul_right (by norm_num)).symm
             _                         ↔ x                         > 14/5             := by ring_nf
             _                         ↔ False                                        := Iff.intro (λ s ↦ absurd (lt_trans s h.right) (by norm_num)) False.elim
             _                         ↔ x <  -14/3 ∨   x >  10/3                     := Iff.intro False.elim (λ s ↦ Or.elim s (λ s1 ↦ absurd (lt_of_le_of_lt h.left s1 ) (by norm_num))
                                                                                                                               (λ s2 ↦ absurd (lt_trans s2 h.right) (by norm_num) ) )
      )
      (
        λ h : x ≥ 2  ↦
          have h1 : 2*x      ≥   0  := le_trans (by norm_num) (mul_le_mul_of_nonneg_left h (by norm_num) )                                              -- by linarith
          ...
          calc
             abs (2*x)-abs (1-x/2) > 6 ↔  abs (2*x)-abs (x/2-1)    > 6         := h0
             _                         ↔   2*x - (x/2-1)           > 6         := by rw[abs_of_nonneg h1, abs_of_nonneg h2 ]
             _                         ↔ 3*x/2+1                   > 6         := by ring_nf
             _                         ↔ 3*x/2+1-1                 > 6-1       := (add_lt_add_iff_right _).symm
             _                         ↔ 3*x/2                     > 5         := by ring_nf
             _                         ↔ 3*x/2*(2/3)               > 5*(2/3)   := (mul_lt_mul_right (by norm_num)).symm
             _                         ↔ x                         > 10/3      := by ring_nf
             _                         ↔ x <  -14/3 ∨   x >  10/3              := Iff.intro Or.inr (λ s ↦ Or.elim s (λ s1 ↦ absurd (lt_of_le_of_lt h s1 ) (by norm_num) ) id  )
      )

    -- Exercice 4.6
    example (x:ℝ ) : abs (2*x)-abs (1-x/2) ≤ -2 ↔ False  :=
      have h0 : abs (2*x)-abs (1-x/2) ≤ -2  ↔  abs (2*x)-abs (x/2-1) ≤ -2 :=
        calc
          abs (2*x)-abs (1-x/2) ≤ -2   ↔ abs (2*x) - (abs (-(1-x/2)))  ≤ -2  := by rw[abs_neg_eq_abs]
          _                            ↔ abs (2*x) - abs (x/2- 1)      ≤ -2  := by ring_nf
      ...
      (
        λ h : x < 0 ↦
          ...
          calc
             ...
             _                          ↔ -3*x/2-1               ≤ -2                := by ring_nf
             _                          ↔ -3*x/2-1 + (3*x/2 + 2) ≤ -2 + (3*x/2 + 2)  := (add_le_add_iff_right _).symm
             _                          ↔  1                     ≤  3*x/2            := by ring_nf
             _                          ↔  1*(2/3)               ≤  3*x/2*(2/3)      := (mul_le_mul_right (by norm_num)).symm
             _                          ↔  2/3                   ≤  x                := by ring_nf
             _                          ↔ False                                      := Iff.intro (λ s ↦ absurd (lt_of_le_of_lt s h) (by norm_num)) False.elim
      )
      (
        λ h : x ≥ 0 ∧ x < 2 ↦
          have h1 : 2*x      ≥  0  := le_trans  (by norm_num)  (mul_le_mul_of_nonneg_left h.left (by norm_num))                                      -- by linarith
          ...
          calc
             ...
             _                          ↔ 5*x/2-1                   ≤ -2               := by ring_nf
             _                          ↔ 5*x/2-1+1                 ≤ -2+1             := (add_le_add_iff_right _).symm
             _                          ↔ 5*x/2                     ≤ -1               := by ring_nf
             _                          ↔ 5*x/2*(2/5)               ≤ -1*(2/5)         := (mul_le_mul_right (by norm_num)).symm
             _                          ↔ x                         ≤ -2/5             := by ring_nf
             _                          ↔ False                                        := Iff.intro (λ s ↦ absurd (le_trans h.left s) (by norm_num)) False.elim
      )
      (
        λ h : x ≥ 2  ↦
          have h1 : 2*x      ≥   0  := le_trans (by norm_num) (mul_le_mul_of_nonneg_left h (by norm_num) )                                              -- by linarith
          ...
          calc
             ...
             _                          ↔ 3*x/2+1                   ≤ -2         := by ring_nf
             _                          ↔ 3*x/2+1-1                 ≤ -2-1       := (add_le_add_iff_right _).symm
             _                          ↔ 3*x/2                     ≤ -3         := by ring_nf
             _                          ↔ 3*x/2*(2/3)               ≤ -3*(2/3)   := (mul_le_mul_right (by norm_num)).symm
             _                          ↔ x                         ≤ -2         := by ring_nf
             _                          ↔ False                                  := Iff.intro (λ s ↦ absurd (le_trans h s) (by norm_num)) False.elim
      )
